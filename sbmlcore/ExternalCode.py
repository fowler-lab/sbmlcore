...
class Stride(object):
    ...
    def __init__(self, PDBFile, dataset_type="mutation", offsets=None):
        ...
        acceptors = pandas.DataFrame(
            hbond_acc, columns=["resname", "segid", "resid", "n_hbond_acceptors"]
        )
        acceptors = acceptors.groupby(["resname", "segid", "resid"]).count()

        donors = pandas.DataFrame(
            hbond_dnr, columns=["resname", "segid", "resid", "n_hbond_donors"]
        )
        ...
        self.results = self.results.astype(
            {"resid": "int", "phi": "float", "psi": "float", "residue_sasa": "float"}
        )
        ...
        if offsets is not None:
            ...
            assert set(offsets.keys()) == set(
                self.results.segid.unique()
            ), "Specified segids in offsets do not match the pdb!"
            ...
            self.results["resid"] = self.results.apply(
                update_resid, args=(offsets,), axis=1
            )

    def _add_feature(self, other, feature_name="all"):
        """
        Private method to add properties measured by stride (e.g. backbone dihedral angles) to existing mutation dataframe
        """
        ...
        assert (
            feature_name not in other.columns
        ), "trying to add a column that is already in the dataset!"

        if feature_name != "all":
            assert (
                feature_name in self.results.columns
            ), "supplied feature_name not supplied by STRIDE"
    ...
    def _add_feature(self, other):
        ...
        if self.offsets is not None:
            for chain in self.offsets:
                assert chain in set(
                    other.segid
                ), "Must only specify the segids in the mutation dataframe! And these must also be present in the pdb."
                assert isinstance(
                    self.offsets[chain], int
                ), "Offsets for each segid must be an integer!"
        else:
            pass
        ...
        other["resname_3"] = [
            sbmlcore.amino_acid_1to3letter[resname] for resname in other.resname_1
        ]
        ...
        sele_text = [
            "%s%i, resi %i and chain %s and resn %s" % (k, i, j, k, l)
            for i, j, k, l in zip(
                other.resid, other.pdb_resid, other.segid, other.resname_3
            )
        ]
        ...
        other.drop(
            columns=["resname_1", "chain_offsets", "resname_3", "pdb_resid", "resid"],
            inplace=True,
        )
...
class SNAP2(object):
    ...
    def __init__(self, CSVFile, offsets=None):
        ...
        no_percentage = snap2_df["Expected Accuracy"].replace(
            to_replace="%", value="", regex=True
        )

        snap2_df["Expected Accuracy"].replace(
        ...
        snap2_df[["alt_amino_acid", "resid"]] = snap2_df.apply(
            split_mutation_toresname, axis=1
        )
    ...
    def _add_feature(self, other):
        ...
        other[["alt_amino_acid", "resid"]] = other.apply(
            split_mutation_toresname, axis=1
        )
