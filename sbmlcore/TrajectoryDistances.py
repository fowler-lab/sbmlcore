...
class TrajectoryDistances(object):
    ...
    @staticmethod
    def _apply_offsets(universe, offsets):
        """returns universe with numbering offsets applied to the atom group"""
        for chain in offsets:
            assert chain in set(
                universe.residues.segids
            ), "Need to specifify a segid that exists in the pdb!"
            assert isinstance(
                offsets[chain], int
            ), "Offsets for each segid must be an integer!"
            chainGroup = universe.select_atoms("segid " + chain)
